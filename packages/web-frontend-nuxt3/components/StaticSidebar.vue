    ...
    <div class="flex flex-col flex-grow pt-5 bg-indigo-700 overflow-y-auto">
      ...
      <div class="mt-5 flex-1 flex flex-col">
        <nav class="flex-1 px-2 pb-4 space-y-1">
          <NuxtLink v-for="item in navigation" :key="item.route" :to="localePath({ name: item.route })" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600', 'group flex items-center px-2 py-2 font-medium rounded-md']">
